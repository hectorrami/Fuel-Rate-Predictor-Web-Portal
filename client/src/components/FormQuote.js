...
const FormQuote = () => {
  ...
  useEffect(() => {
    FormService.getForm().then((data) => {
      if (data.form != null) {
        setHistoryRate({ ...historyRate, rate_for_history: 0.01 });
      } else {
        console.log("ERROR: NEED A PROFILE ADDRESS");
      }
    });
  ...
  const priceModule = () => {
    ...
    if (
      currDate >= new Date("2020/05/01") &&
      currDate <= new Date("2020/08/30")
    ) {
    ...
    <div>
      <Form className="quoteForm">
        <FormGroup>
          ...
          />
          {isError ? (
            <p className="text-danger"> Please enter a valid gallon value</p>
          ) : (
            []
          )}
          ...
          />
          {dateError ? (
            <p className="text-danger">Please enter a correct date</p>
          ) : (
            []
          )}
          ...
          <Input
            ...
            disabled
          />
          <Button type="submit" onClick={onSubmit} className="mb-3" block>
            Submit Quote
          </Button>
        </FormGroup>
      </Form>
